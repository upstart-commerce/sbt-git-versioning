...
class GitDriverImplSpec extends FunSpec {
  ...
  describe("gitState") {
    ...
    it("commitCount") {
      // did it do something? I don't care what it did really, as long as it didn't throw
      val driver = new GitDriverImpl(workingDir)
      assert(driver.getCommitCount(None, true) > 1)
    }
  ...
  describe("getCommitCount") {
    ...
    it("no limit (count to head)") {
      val commitCount = driver.getCommitCount(None, true)
    ...
    it("limit (count back 3)") {
      ...
      val commitCount = driver.getCommitCount(prevHash, true)
  ...
  describe("calcCurrentVersion") {
    ...
    describe("some commit is tagged") {
      ...
      describe("GitBranchStateNoCommits") {
        ...
        it("dirty") {
          ...
          val result = gitDriver.calcCurrentVersion(ignoreDirty = false, true)
    ...
    describe("ignoreDirty") {
      ...
      it("GitBranchStateNoCommits") {
        ...
        val workingState = GitWorkingState(isDirty = true)
        val gitDriver = MockDriver(branchState, workingState)
        val result = gitDriver.calcCurrentVersion(ignoreDirty = true, true)
...
object GitDriverImplSpec {
  ...
  case class MockDriver(branchState: GitBranchState, workingState: GitWorkingState) extends GitDriver {
    ...
    override def getCommitCount(hash: Option[String], firstParent: Boolean): Int = throw new UnsupportedOperationException

    override def branchState(firstParent: Boolean): GitBranchState = branchState
